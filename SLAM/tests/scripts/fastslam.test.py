...
def do_fast_slam(xEst, xTrue, xDR, hxEst, hxTrue, hxDR, u, motion, frame, particles, built_map, TRACK):
    # xTrue, z, xDR, ud = observation(xTrue, xDR, u, TRACK) # meter dx y dx directamente en vez de track ////z = np.hstack((z, zi))
    # frame --> [[cone1.dx, cone1.dy], [cone2.dx, cone2.dy], ...]
    ...
    if show_animation:  # pragma: no cover
        ...
        for iz in range(len(z[:, 0])):
            ...
            plt.plot([xEst[0], TRACK[landmark_id, 0]], [
                xEst[1], TRACK[landmark_id, 1]], "-k")
...
def main():
    ...
    frames_file = 'inputs/perception.pkl'
...
#    STATES = [] # [[x1 y1 v1 yaw1],[x2 y2 v2 yaw2],..., [xn yn vn yawn]] # Where n is the number of frames
    motions_file = 'inputs/motions.pkl'
    ...
    for frame, motion in zip(FRAMES, MOTIONS):
        n_cones = len(frame)
        particles = [Particle(n_cones, motion[0], motion[1], motion[3])
                     for _ in range(N_PARTICLES)]
        time += 0.033
        u = calc_input(time)
        state, final_map = do_fast_slam(
            xEst, xTrue, xDR, hxEst, hxTrue, hxDR, u, motion, frame, particles, built_map, TRACK)
